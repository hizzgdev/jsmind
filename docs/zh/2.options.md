...
实例化一个 jsMind 时，此参数不可缺省。jsMind 通过此参数查找页面元素，并将思维导图输出到该元素中。为了便于控制思维导图的大小和位置，请使用块元素[(CSS块级元素、内联元素概念)][1]作为思维导图的容器，一般来说可以使用`<div>`作为容器。
...
[1]http://www.nowamagic.net/librarys/veda/detail/1190
