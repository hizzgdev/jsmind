...
; (function ($w) {
    ...
    jm.prototype = {
        ...
        add_nodes: function (parent_node, nodes_data) {
            if (!this.get_editable()) {
                logger.error('fail, this mind map is not editable');
                return [];
            }
            ...
            for (var i = 0; i < nodes_data.length; i++) {
                ...
                var dir = jm.direction.of(node_data.direction);
                if (dir === undefined) {
                    dir = this.layout.calculate_next_child_direction(the_parent_node);
                }
            ...
            for (var j = 0; j < created_nodes.length; j++) {
                if (created_nodes[j] !== null) {
                    has_success = true;
                    break;
                }
            }
            ...
            if (has_success) {
                ...
                // Trigger batch add event
                var success_nodes = [];
                for (var k = 0; k < created_nodes.length; k++) {
                    if (created_nodes[k] !== null) {
                        success_nodes.push(created_nodes[k].id);
                    }
                }
