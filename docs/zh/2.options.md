...
**view.line_color** : (string) 思维导图线条的颜色（html的颜色表示方法）。[仅在 ES6 版本里支持] 如果一个 `node` 设置了 `data.leading-line-color`，此选项将会被覆盖。
**view.line_style** : (string) 思维导图线条的样式. [仅在 ES6 版本里支持], 可用的选项有 `straight` 表示直线，或者 `curved` 表示曲线。
